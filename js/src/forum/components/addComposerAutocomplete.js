...
export default function addComposerAutocomplete() {
  extend(QuillEditor.prototype, 'oncreate', function () {
    const $container = $('<div class="ComposerBody-mentionsDropdownContainer"></div>');
    const dropdown = new AutocompleteDropdown();
    const $editor = this.$('.quill-editor').wrap('<div class="ComposerBody-mentionsWrapper"></div>');
    const $textarea = this.$('.quill-editor > .ql-editor');
    const searched = [];
    let mentionStart;
    let typed;
    let searchTimeout;
    ...
    const applySuggestion = (replacement) => {
      const cursor = app.composer.editor.getLength() - 1;
      app.composer.editor.deleteText(mentionStart - 1, cursor);
      app.composer.editor.insertText(mentionStart - 1, replacement + ' ');
    ...
    $textarea
      .on('click keyup input', function (e) {
        ...
        const selection = app.composer.editor.getSelection(true);
        this.selectionStart = !selection.length ? selection.index : app.composer.editor.getLength();
        this.selectionEnd = !selection.length ? selection.index + selection.length : app.composer.editor.getLength();
        ...
        if (mentionStart) {
          ...
          const buildSuggestions = () => {
            ...
            if (suggestions.length) {
              ...
              dropdown.show();
              const coordinates = app.composer.editor.getBounds(mentionStart);
  ...
  extend(QuillEditor.prototype, 'toolbarItems', function (items) {
    items.add('mention', (
      <TextEditorButton onclick={() => {
        const cursor = this.attrs.composer.editor.getLength() - 1;
        this.attrs.composer.editor.insertText(cursor, '@', 'user');
        this.attrs.composer.editor.focus();
      }} icon="fas fa-at">
